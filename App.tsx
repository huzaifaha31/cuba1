...
const App: React.FC = () => {
  ...
  // Effect to scroll to top on view change
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [currentView]);

  // Helper to determine grid classes for team members
  const getTeamMemberClass = (index: number) => {
    ...
    // Index 0: Supervisor - Center on top row (Desktop)
    if (index === 0) {
      return `${baseClass} lg:col-start-2`;
    }
    
    // Index 1: Ahmad - Start new row (Desktop) to ensure Supervisor is alone on top
    if (index === 1) {
      return `${baseClass} lg:col-start-1`;
    }
    
    // Index 4: Nurul - Center on bottom row
    // Tablet: Span 2 cols, width 50%, centered
    // Desktop: Center column (col-start-2)
    if (index === 4) {
      return `${baseClass} md:col-span-2 md:w-1/2 md:mx-auto lg:w-full lg:col-span-1 lg:col-start-2 lg:mx-0`;
    }
        ...
        {currentView === 'BROWSE' && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen">
            <div className="text-center mb-12 animate-fade-in-up">
              <h2 className="font-display text-4xl font-bold text-manuscript-brown mb-4">Browse The Archive</h2>
              <p className="text-lg font-serif italic text-manuscript-dark/70 max-w-2xl mx-auto">
                Search through the catalog of ailments to discover their traditional treatments as recorded in the manuscripts.
              </p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-10 justify-center items-center">
              ...
              <div className="flex overflow-x-auto pb-2 md:pb-0 gap-2 w-full md:w-auto px-2">
                {categories.map(cat => (
                  <button
                    key={cat}
          ...
          <div className="max-w-6xl mx-auto px-4 py-16 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {TEAM_MEMBERS.map((member, index) => (
                <div 
                  key={index} 
                  className={getTeamMemberClass(index)}
                >
                  <div className="p-8 text-center flex flex-col items-center">
                    <div className="w-32 h-32 rounded-full border-4 border-manuscript-paper shadow-inner overflow-hidden mb-6 relative">
                       {/* Fallback pattern if image fails to load, though we have placeholders */}
                       <div className="absolute inset-0 bg-manuscript-gold/20 flex items-center justify-center text-manuscript-brown/30">
                         <Users size={40} />
                       </div>
                       <img 
                        src={member.imageUrl} 
                    ...
                    <h3 className="font-display text-xl font-bold text-manuscript-dark mb-2">
                      {member.name}
                    </h3>
                    
                    <div className="inline-flex items-center gap-2 bg-manuscript-paper/50 px-3 py-1 rounded-full text-xs font-bold text-manuscript-brown uppercase tracking-widest mt-2">
                       {member.role.includes("Supervisor") ? <GraduationCap size={14} /> : null}
                       {member.role}
                    </div>
                  </div>
                </div>
